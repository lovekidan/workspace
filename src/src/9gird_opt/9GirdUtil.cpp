...
uint64_t PicOpt::Utility::WriteToPngFile(const ImageDesc &img_out){
	...
	return std::tr2::sys::file_size(std::tr2::sys::path(img_out.name));
}
...
std::tr2::sys::path PicOpt::Utility::MakeRelative(std::tr2::sys::path path, std::tr2::sys::path file)
{
	...
	path = std::tr2::sys::complete(path);
	file = std::tr2::sys::complete(file);
	...
	return std::tr2::sys::path(Utf16ToUtf8(std::wstring(buffer.data())));
}
